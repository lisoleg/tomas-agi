...
### Added
- **v3.14 五大理论模块 + 25 个 REST 端点**：
  - **叠加态几何** (`superposition_geometry.py`)：Thomson问题球面排布、E8格最优堆积、相变检测、特权基分析、对抗脆弱性度量；41 项自测全通过
  - **数学大统一与计算共形几何** (`math_unification_ccg.py`)：热带半环骨架、离散Ricci流、UV/IR对偶引擎、Gromov-Wasserstein柏拉图趋同、太一几何定位表；50 项自测全通过
  - **自适应库学习** (`adaptive_library.py`)：α/β在线学习、Sleep-Step子树归纳、阴龙积八元数耦合、条件ΔT归纳、Rice定理上界；55 项自测全通过
  - **Curry-Howard-Lambek 同构** (`chl_isomorphism.py`)：命题↔证明项↔范畴态射三角映射、κ-Snap证明搜索、Sleep-Step公理扩展；48 项自测全通过
  - **太一互搏Agent** (`taiyi_mutual_duel.py`)：L3差分感知、L2 DFS回溯博弈规划、L4贝叶斯熔断、博弈语义引擎、Physical AI定理；48 项自测全通过
  - 25 个新 Flask REST 端点（5 模块 × 5 端点），Flask 端点总数 168 → 193
  - 理论参考：微信公众号文章《NASGA中的均匀超图谱》——谱折叠、κ-截断、结合子归零退化的均匀超图谱理论为五大模块提供统一代数基础
- 新增 5 个模块文件，~4,800 行代码
- 累计模块数 97+ → 102+，累计测试数 1368 → 1610+（新增 242 项自测）

### Changed
- README.md：更新模块数（97+→102+）、测试数（1368→1610+）、端点数（168→193）、新增 5 模块描述
- ARCHITECTURE.md：版本号 v3.13 → v3.14，新增 5 模块架构定位
- api_v2.md：版本号 v2.1 → v2.2，新增 25 个 v3.14 端点文档

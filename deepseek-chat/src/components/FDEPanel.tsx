...
export default function FDEPanel() {
  ...
  return (
    <div className="flex-1 overflow-y-auto p-6 space-y-6">
      <h2 className="text-xl font-bold text-white mb-4">FDE 道法术器本体构建器</h2>
      ...
      {tab === 'build' && (
        <div className="bg-slate-800/60 rounded-xl p-5 border border-slate-700/50 space-y-4">
          <div>
            <label className="block text-xs text-slate-400 mb-1">IT 上下文</label>
      ...
      {tab === 'calibrate' && (
        <div className="bg-slate-800/60 rounded-xl p-5 border border-slate-700/50 space-y-4">
          <div>
            <label className="block text-xs text-slate-400 mb-1">目标概念</label>
            <input value={calConcept} onChange={e => setCalConcept(e.target.value)}
              className="w-full bg-slate-900 border border-slate-700 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-indigo-500"
              placeholder="e.g. momentum, entropy, attention" />
          </div>
          <div>
            <label className="block text-xs text-slate-400 mb-1">ℐ 值: {calIValue.toFixed(2)}</label>
      ...
      {tab === 'asym' && (
        <div className="bg-slate-800/60 rounded-xl p-5 border border-slate-700/50 space-y-4">
          <div>
            <label className="block text-xs text-slate-400 mb-1">技能描述</label>
      ...
      {result && (
        <div className="bg-slate-800/60 rounded-xl p-5 border border-slate-700/50 space-y-3">
          ...
          <pre className="text-xs text-slate-300 bg-slate-900/50 rounded-lg p-3 overflow-x-auto whitespace-pre-wrap">
